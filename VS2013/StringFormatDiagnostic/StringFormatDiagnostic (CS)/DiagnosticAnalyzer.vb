...
Public Class DiagnosticAnalyzer
  ...
  Public ReadOnly Property SupportedDiagnostics As ImmutableArray(Of DiagnosticDescriptor) Implements IDiagnosticAnalyzer.SupportedDiagnostics
    ...
    End Get
  End Property
  Private ReadOnly Property SyntaxKindsOfInterest As ImmutableArray(Of SyntaxKind) Implements ISyntaxNodeAnalyzer(Of Microsoft.CodeAnalysis.CSharp.SyntaxKind).SyntaxKindsOfInterest
    Get
  ...
  Public Sub AnalyzeNode(node As SyntaxNode, semanticModel As SemanticModel, addDiagnostic As Action(Of Diagnostic), cancellationToken As CancellationToken) Implements ISyntaxNodeAnalyzer(Of Microsoft.CodeAnalysis.CSharp.SyntaxKind).AnalyzeNode
  ...
  Private Sub _Shared_Checker_(fn As Func(Of CancellationToken,
    ...
    If FSIndex < 0 Then Exit Sub Dim p = CType(node.Parent, InvocationExpressionSyntax)
          ...
          Select Case TheFormatString.Expression.CSharpKind
            ...
            Case SyntaxKind.IdentifierName
              ...
              If FoundSymbol.IsExtern Then
                ...
                Dim ReportedIssues = fn(ct, ConstValue.Value.ToString, 1, ifp, Args)
                For Each ReportedIssue In ReportedIssues.Errors
                  If TypeOf ReportedIssue Is Interfaces.IReportIssueWithPositionAndLength Then
                    Dim ir = DirectCast(ReportedIssue, Interfaces.IReportIssueWithPositionAndLength)
                    Select Case ReportedIssue.Level
                      ...
                      Case DiagnosticSeverity.Warning
                        diag(AddWarningAtSource(TheValueOfTheVariable, ir.Index, ir.Index + ir.Length, ReportedIssue))
                      Case DiagnosticSeverity.Error
                        diag(AddErrorAtSource(TheValueOfTheVariable, ir.Index, ir.Index + ir.Length, ReportedIssue))
                      'Case DiagnosticSeverity.Hidden
                    End Select
                  ElseIf TypeOf ReportedIssue Is Interfaces.IReportIssue Then
                    diag(AddInformation(TheValueOfTheVariable, ReportedIssue.Message))
