...
Namespace TestHelper
  ...
  Partial Public MustInherit Class CodeFixVerifier
    ...
    Protected Overridable Function GetCSharpCodeFixProvider() As ICodeFixProvider
      Return Nothing
    End Function
    ...
    Protected Overridable Function GetBasicCodeFixProvider() As ICodeFixProvider
      Return Nothing
    End Function
    ...
    ''' <param name="codeFixIndex">Index determining which codefix to apply if there are multiple</param>
    ''' <param name="allowNewCompilerDiagnostics">A bool controlling whether Or Not the test will fail if the CodeFix introduces other warnings after being applied</param>
    Private Sub VerifyFix(language As String, analyzer As IDiagnosticAnalyzer, codeFixProvider As ICodeFixProvider, oldSource As String, newSource As String, codeFixIndex As Integer?, allowNewCompilerDiagnostics As Boolean)
      ...
      For i = 0 To attempts - 1
        Dim actions = codeFixProvider.GetFixesAsync(document, analyzerDiagnostics(0).Location.SourceSpan, analyzerDiagnostics, CancellationToken.None).Result
