...
Namespace TestHelper
    ...
    Partial Public MustInherit Class DiagnosticVerifier
        ...
        Private Shared ReadOnly CorlibReference As New MetadataFileReference(GetType(Object).Assembly.Location, MetadataImageKind.Assembly)
        Private Shared ReadOnly SystemCoreReference As New MetadataFileReference(GetType(Enumerable).Assembly.Location, MetadataImageKind.Assembly)
        Private Shared ReadOnly VisualBasicSymbolsReference As New MetadataFileReference(GetType(VisualBasicCompilation).Assembly.Location, MetadataImageKind.Assembly)
        Private Shared ReadOnly CodeAnalysisReference As New MetadataFileReference(GetType(Compilation).Assembly.Location, MetadataImageKind.Assembly)
...
#Region " Get Diagnostics "
        ...
        Private Shared Function GetSortedDiagnostics(sources As String(), language As String, analyzer As IDiagnosticAnalyzer) As Diagnostic()
        ...
        ''' <returns>An IEnumerable of Diagnostics that surfaced in the source code, sorted by Location</returns>
        Protected Shared Function GetSortedDiagnosticsFromDocuments(analyzer As IDiagnosticAnalyzer, documents As Document()) As Diagnostic()
            ...
            For Each project In projects
                ...
                Dim driver = New AnalyzerDriver(Of SyntaxKind)((New IDiagnosticAnalyzer() {analyzer}).ToImmutableArray, Function(n) n.VisualBasicKind, Nothing, CancellationToken.None)
