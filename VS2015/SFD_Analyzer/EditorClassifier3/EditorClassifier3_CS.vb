...
Imports RoslynExts
Imports RoslynExts.CS
Imports Microsoft.CodeAnalysis.CSharp.Syntax
Imports EditorClassifier3.Exts_CS
Imports EditorClassifier3
...
<ContentType("CSharp")>
Class EditorClassifier3Provider_CS
  ...
  Public Function GetClassifier(ByVal buffer As ITextBuffer) As IClassifier Implements IClassifierProvider.GetClassifier
    Return buffer.Properties.GetOrCreateSingletonProperty(Of EditorClassifier3_CS)(Function() New EditorClassifier3_CS(_classificationRegistry))
  End Function
...
Class EditorClassifier3_CS
  ...
  Public Function GetClassificationSpans(ByVal span As SnapshotSpan) As IList(Of ClassificationSpan) Implements IClassifier.GetClassificationSpans
    ...
    For Each mi In rev.AsParallel
      ...
      Dim TheFormatString = _Arg_.As(Of ArgumentSyntax)
...
Public Module Exts_CS

  <Extension>
  Private Function CalledOnType(n As MemberAccessExpressionSyntax, context As SyntaxNodeAnalysisContext) As INamedTypeSymbol
  ...
  End Function
  <Extension>
  Private Function CalledOnType(n As MemberAccessExpressionSyntax, sm As SemanticModel) As INamedTypeSymbol
    If (n Is Nothing) Then Return Nothing
  ...
  End Function
  <Extension>
  Private Function ToFullyQualifiedName(s As ISymbol) As String
  ...
  <Extension>
  Private Function MakeClassSpan(ss As ITextSnapshot, fs As SyntaxNode, p1 As Integer, p2 As Integer, _color_ As IClassificationType) As ClassificationSpan
    Dim size = p2 - p1
    Return New ClassificationSpan(New SnapshotSpan(New SnapshotPoint(ss, fs.SpanStart + p1 + 1), size - 0), _color_)
  End Function
  Private Function MakeClassSpan(ss As ITextSnapshot, fs As CSharp.Syntax.ArgumentSyntax, p1 As Integer, _color_ As IClassificationType) As ClassificationSpan
