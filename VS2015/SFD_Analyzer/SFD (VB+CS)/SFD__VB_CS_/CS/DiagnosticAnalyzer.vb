...
Imports System.Text
Imports AdamSpeight2008.StringFormatDiagnostics.CSharp
...
Public Class SFD_CS_Analyzer
  ...
  Dim _C_ As Checkers_CS


  Public Overrides Sub Initialize(context As AnalysisContext)
    SFD.Analysers.Initialise()
    _C_ = New Checkers_CS(_DI_, SFD.Analysers.DAnalysers)
    context.RegisterSyntaxNodeAction(AddressOf AnalyzeSymbol, SyntaxKind.SimpleMemberAccessExpression)
  End Sub

  Sub AnalyzeSymbol(context As SyntaxNodeAnalysisContext)
    ...
    ' Is the operator '.' ?
    If method.OperatorToken.CSharpKind <> SyntaxKind.DotToken Then Exit Sub
    ...
    For Each qp As AB In possibles.AsParallel.Where(Function(p) _ArgTypeNames_.BeginsWith(p.ParamTypes) AndAlso
  ...
  Namespace CSharp
    Public Module Exts
      ...
      Public Function ArgumentType(arg As ArgumentSyntax, context As SyntaxNodeAnalysisContext) As ITypeSymbol
        Try
          Return context.SemanticModel.GetTypeInfo(CType(arg, ArgumentSyntax).Expression, context.CancellationToken).Type
